...
async function run() {
  ...
  try {
    ...
    const users = [
      { name: 'Bob Doe', age: 30, skills: ['HTML', 'Node.js', 'CSS', 'Node.js', 'JS', 'Node.js'] },
      { name: 'Jane Woo', age: 25, skills: ['Python', 'Django', 'Node.js'] },
    ...
    const updateManyResult = await db.collection('users').updateMany({}, { $pull: { skills: 'Node.js' } })
