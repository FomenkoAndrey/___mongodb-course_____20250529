...
async function run() {
  ...
  try {
    ...
    const users = [
      ...
      { name: 'Jack Daniels', age: 80, skills: ['C#', '.NET'] },
      { name: 'Jonny Walker', age: 65, skills: ['JavaScript', 'React'] }
    ]
    ...
    const updateManyResult = await db.collection('users').updateMany({}, { $max: { age: 50 } })
