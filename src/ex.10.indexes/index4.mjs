...
async function run() {
  ...
  try {
    ...
    const sortByFirstNameAsc = await usersCollection
      .find()
      .sort({ 'person.first': 1 })
      .limit(3)
      .toArray()
    console.log(chalk.magentaBright('Top 3 sorted by first name in ascending order:'), sortByFirstNameAsc)

    const sortByFirstNameDesc = await usersCollection
      .find()
      .sort({ 'person.first': -1 })
      .limit(3)
      .toArray()
    console.log(chalk.magentaBright('Top 3 sorted by first name in descending order:'), sortByFirstNameDesc)
